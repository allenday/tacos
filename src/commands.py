...
def handle_stats_command(ack, body, client):
    ...
    if events:
        ...
        for i, event in enumerate(events):
            ...
            notes = event.get('notes', '').split(',')
            givers = event.get('givers', '').split(',')
            recipients = event.get('recipients', '').split(',')
            
            primary_note = next((note for note in notes if note and not note.startswith('Reaction')), "No reason provided")
            ...
            message += f"{i+1}. <{message_link}|{primary_note}>: {reaction_count} reactions from {len(unique_givers)} people\n"
            
            if len(unique_givers) > 0:
                message += f"   *Given by:* "
                giver_counts = {}
                for giver in givers:
